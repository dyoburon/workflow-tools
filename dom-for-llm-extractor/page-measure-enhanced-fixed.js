(function() {
    ...
    function getPositionInParent(el) {
        ...
        var leftPct = ((elRect.left - parentRect.left) / parentRect.width * 100).toFixed(1);
        var topPct = ((elRect.top - parentRect.top) / parentRect.height * 100).toFixed(1);
    ...
    function buildOutput(rect, elements) {
        ...
        var widthPct = ((rect.right - rect.left) / vw * 100).toFixed(1);
        var heightPct = ((rect.bottom - rect.top) / vh * 100).toFixed(1);
        ...
        if (elements.length === 0) {
            ...
            var text = getText(primary.el);
            if (text) {
                lines.push('Text: ' + text);
            }
            lines.push('Selector: ' + getSelector(primary.el));
            var posInParent = getPositionInParent(primary.el);
            if (posInParent) {
            ...
            if (elements.length > 1) {
                ...
                for (var i = 1; i < Math.min(elements.length, 6); i++) {
                    var el = elements[i];
                    var elText = getText(el.el);
                    var textPart = elText ? ' ' + elText.substring(0, 30) + (elText.length > 30 ? '...' : '') : '';
    ...
    function onMouseUp(e) {
        ...
        navigator.clipboard.writeText(output)
            .then(function() { showToast('Copied!' + String.fromCharCode(10) + String.fromCharCode(10) + output.substring(0, 200) + (output.length > 200 ? '...' : ''), 3000); })
